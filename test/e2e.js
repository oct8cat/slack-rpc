...
describe('SlackRPC', function () {
    ...
    describe('POST /', function () {
        it('Should deny no-trigger-word requests', function (done) {
            agent.post('/').send(fixtures.requests[0]).expect(200, function (err, res) {
                if (err) { done(err); return }
        ...
        it('Should deny unknown-trigger-word requests', function (done) {
            agent.post('/').send(fixtures.requests[1]).expect(200, function (err, res) {
                if (err) { done(err); return }
        ...
        it('Should execute valid requests', function (done) {
            agent.post('/').send(fixtures.requests[2]).expect(200, function (err, res) {
                if (err) { done(err); return }
